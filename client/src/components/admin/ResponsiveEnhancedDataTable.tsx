...
interface ResponsiveEnhancedDataTableProps<T> {
  data: T[];
  columns: Column<T>[];
  title: string;
  description?: string;
...
export function ResponsiveEnhancedDataTable<T>({
  data,
  columns,
  title,
  description,
  ...
  onViewModeChange,
  renderCard,
}: ResponsiveEnhancedDataTableProps<T>) {
        ...
        <div className="flex flex-col gap-3 sm:gap-4">
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 sm:gap-4">
            <div className="min-w-0 flex-1">
              <CardTitle className="text-base sm:text-lg lg:text-xl text-white flex items-center gap-2 flex-wrap">
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-3">
            {onViewModeChange && (
              <div className="flex border border-gray-600 rounded-md overflow-hidden">
                <Button
                  ...
                  className="rounded-none border-0 px-2 sm:px-3 h-8 sm:h-9"
                >
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              {onExport && (
                <Button variant="outline" size="sm" onClick={onExport} className="px-2 sm:px-3 h-8 sm:h-9">
      ...
      <CardContent className="p-3 sm:p-6 pt-0">
        ...
        <div className="flex flex-col gap-2 sm:gap-3 mb-4 sm:mb-6">
          <div className="relative">
            <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-500 h-3 w-3 sm:h-4 sm:w-4" />
            <Input
              placeholder={searchPlaceholder}
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="pl-8 sm:pl-9 bg-gray-900 border-gray-600 text-white text-sm h-9 sm:h-10"
            />
          </div>
          ...
          {filterableColumns.length > 0 && (
            <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
              <Select value={filterBy?.toString() || "all"} onValueChange={(value) => setFilterBy(value === "all" ? null : value as keyof T)}>
                <SelectTrigger className="w-full sm:w-40 bg-gray-900 border-gray-600 h-9 sm:h-10">
              ...
              {filterBy && (
                <Input
                  ...
                  className="w-full sm:w-40 bg-gray-900 border-gray-600 text-white h-9 sm:h-10"
                />
              )}
            </div>
          )}
          ...
          <>
                        ...
                        {columns.filter(col => col.key !== "actions").slice(0, 4).map((column) => {
                          ...
                          return (
                            <div key={column.key.toString()} className="flex justify-between items-start">
                              <span className="text-xs sm:text-sm font-medium text-gray-400 min-w-0 flex-1">
                                {column.label}:
                              </span>
                              <span className="text-xs sm:text-sm text-white ml-2 text-right truncate">
                                {value || "—"}
                              </span>
                            </div>
                          );
            ...
            <div className="hidden lg:block rounded-md border border-gray-600 overflow-hidden mb-6">
              <div className="overflow-x-auto">
                <Table>
                  <TableHeader>
                    <TableRow className="bg-gray-700/50 hover:bg-gray-700/50 border-gray-600">
                      {columns.map((column) => (
                        <TableHead
                          key={column.key.toString()}
                          className={`text-gray-300 font-semibold whitespace-nowrap text-xs ${column.width || ""} ${
                  ...
                  <TableBody>
                    {paginatedData.length > 0 ? (
                      paginatedData.map((item, index) => (
                        ...
                        >
                          {columns.map((column) => (
                            <TableCell key={column.key.toString()} className="text-gray-300 text-xs py-3">
                              {column.key === "actions" ? (
                                <div className="flex items-center gap-1">
                                  ...
                                  {onEdit && (
                                    <Button
                                      ...
                                      size="sm"
                                      onClick={() => onEdit(item)}
                                      className="h-7 w-7 p-0 hover:bg-blue-600"
                                    >
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-1 order-1 sm:order-2">
              <Button
                ...
                className="bg-gray-900 border-gray-600 h-7 w-7 p-0"
              >
